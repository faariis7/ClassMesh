...
PR #168 makes the sensitive grant itself one-shot over `ControlChannel`: the wrapper is consumed and its decoded protobuf key bytes are zeroized before return on success or transport failure. Receiver-side ACK construction is derived only from the same already-installed grant envelope, preserving exact session/version/request/presentation/stream/epoch correlation after the raw key buffer has been zeroized.

PR #169 wires that receiver contract into the authenticated Student Service session. `SframeGroupMedia` is advertised only with the handler present; a grant must pass negotiated-capability, live `StartPresentation` authorization and exact active Teacher principal/control-session/presentation/stream ownership checks. Within a live presentation, epochs must increase strictly. Every pre-install rejection explicitly zeroizes decoded key bytes, successful install emits the exact correlated ACK, and only derived SFrame receiver/replay state is retained until stop/session teardown. The current Teacher-side binding slice resolves the TLS-verified Student Service server certificate through the live authorization store, requires a stable `StudentDevice` PrincipalId plus Teacher role and the negotiated v0.4 group-media contract, and binds that identity to the exact bundled client control session. Bounded Teacher-side grant send/ACK session management remains before 7E is complete.
