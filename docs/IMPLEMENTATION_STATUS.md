...
- Phase 6 adapter/driver/encoder/profile cache-identity prerequisites for measured H.264 evidence, followed by capability-cache/runtime integration and reconfigure validation, production focused encoder/sender integration, and physical interactive-control validation under Issue #108;
...
2. Finish the adapter/driver/encoder/profile cache-identity slice for measured H.264 evidence, then add bounded persistence/invalidation before runtime `H264HardwareEncode` can be advertised; dynamic reconfigure remains separately unproven.
3. Continue the remaining Phase 6D production focused encoder/sender integration on top of the merged profile plumbing and configurable GPU H.264 target pipeline, without selecting UDP vs QUIC Datagram before Issue #3 physical evidence.
