...
- Phase 6 durable encoder-evidence persistence in PR #130, followed by exact-key Worker load/re-probe integration and reconfigure validation, production focused encoder/sender integration, and physical interactive-control validation under Issue #108;
...
2. Finish PR #130 bounded encoder-evidence persistence, then wire exact-key cache load/invalidation and re-probe into the active Worker generation before runtime `H264HardwareEncode` can be advertised; dynamic reconfigure remains separately unproven.
