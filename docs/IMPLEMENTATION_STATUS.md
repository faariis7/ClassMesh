...
PR #51 merged enrolled-QUIC mTLS on the current baseline with CI #288 green. It requires caller-supplied client-certificate verification on the server and resolver-backed client credentials so protected keys do not need DER export.

PR #52 is the active X.509 issuance slice. It signs verified approved PKCS#10 requests through rcgen's `SigningKey` abstraction, forces end-entity CA/key-usage constraints instead of trusting CSR-requested privilege, and returns the issued leaf DER/fingerprint together with the exact stable PrincipalId + CSR binding. Authority-provider integration and transport-connected revocation/rotation enforcement still remain before Phase 5C3 is complete.
...
2. Finish PR #52 bounded X.509 issuance against the explicit bootstrap trust and approved CSR binding.
3. Integrate the authority signing provider without private-key export, then enforce revocation/rotation on authenticated transport sessions.
