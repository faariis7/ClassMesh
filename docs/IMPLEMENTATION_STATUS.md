...
PR #66 merged the first Phase 5F compatibility-hardening slice after CI #338 green. It rejects missing or internally inconsistent Hello protocol versions before negotiation/session establishment while preserving the enrolled mTLS identity binding.

PR #69 merged malformed-control-Protobuf regression coverage after CI #343 green, proving a correctly length-framed but invalid Protobuf payload fails closed as `FrameError::Decode`.

### Phase 5F — hardening active

PR #70 is active on current `main` to validate HelloAck envelope session/request/sequence/version consistency before the client accepts a negotiated control session. Remaining Phase 5F work includes reconnect/timeout edge cases, broader malformed authenticated-envelope coverage, fuzz targets, and security/compatibility diagnostics.
