...
PR #45 merged the certificate-issuance policy slice. It requires exact approved PrincipalId/CSR binding and bounded validity before an authority may issue a credential, while rejecting empty/oversized CSRs, expired windows and excessive future skew/lifetime. Main CI #272 passed after merge. PR #46 now adds PKCS#10 proof-of-possession verification, and PR #47 adds an explicit bounded credential-rotation overlap path; both remain subject to their own CI before merge.
...
2. Finish PR #46 PKCS#10 verification and PR #47 bounded rotation overlap, then integrate X.509 signing against the explicit bootstrap trust model merged in PR #43.
