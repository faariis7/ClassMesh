...
10. **Phase 7 wired-classroom Teacher Presentation — active in parallel with physical gates.** 7A transport-neutral lifecycle landed in PR #154; 7B bounded authenticated ownership/session cleanup landed in PR #156/#157; 7C bounded shared encoded fan-out landed in PR #158; 7D multicast lifecycle/probe implementation landed in PR #159/#160 with physical multicast viability still unproven. The RFC 9605 SFrame crypto core landed in PR #161 and the bounded `ReceivePresentation`-gated receiver/key coordinator landed in PR #162. The current 7E wire slice adds protocol v0.4 `PresentationKeyGrant`/`PresentationKeyAck`, exact 32-byte key validation and explicit `SframeGroupMedia` capability negotiation while deliberately keeping the sensitive payload outside the existing privileged-command dispatcher until receiver/session semantics are integrated. Production multicast wiring remains a later slice. 7H classroom-scale physical qualification remains required before production multicast media.
