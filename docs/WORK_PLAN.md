...
10. **Phase 7 wired-classroom Teacher Presentation — active in parallel with physical gates.** 7A transport-neutral lifecycle landed in PR #154; 7B bounded authenticated ownership/session cleanup landed in PR #156/#157; 7C bounded shared encoded fan-out landed in PR #158; 7D multicast lifecycle/probe implementation landed in PR #159/#160 with physical multicast viability still unproven. The RFC 9605 SFrame crypto core landed in PR #161; the current 7E slice adds bounded `ReceivePresentation`-gated receiver membership, exact epoch acknowledgement, and fail-closed rotation on membership/authorization changes. Authenticated control-wire key delivery and production multicast wiring remain later slices. 7H classroom-scale physical qualification remains required before production multicast media.
