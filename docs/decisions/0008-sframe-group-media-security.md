...
- Sender-side delivery is created only for a TLS-authenticated `StudentDevice` peer with the v0.4 group-media capability contract. Server-side enrolled sessions bind from the enrolled handshake result; in the current Teacher→Student Service topology, the Teacher client resolves the verified Student Service server certificate to the stable StudentDevice PrincipalId through the live authorization store. ACK acceptance reuses the authenticated control guard and rechecks live `ReceivePresentation` authorization before installation.
...
- The Student Service receiver advertises `SframeGroupMedia` only with its authenticated key handler present. A received grant requires live `StartPresentation` authorization for the authenticated Teacher plus an exact active Teacher principal/control-session/presentation/stream ownership match; replacement epochs within that live binding must increase strictly.
- Every Student Service pre-install rejection zeroizes decoded protobuf key bytes. A successful install emits the exact correlated ACK and retains only derived SFrame receiver/replay state until presentation stop or session teardown.
- The Student Service remains fail-closed for group-key sender behavior because its authenticated peer is the Teacher. The sender binding layer is intended for the Teacher-side control runtime where the authenticated peer is the StudentDevice receiver.
