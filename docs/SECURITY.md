...
14. sender-side delivery is bound to the server-side enrolled mTLS peer identity + exact control session, requires `StudentDevice` role and the v0.4 group-media capability contract, and carries no payload-supplied recipient identity;
